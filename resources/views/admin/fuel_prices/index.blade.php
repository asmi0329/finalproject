    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-10">
                <div>
                    <h1 class="text-3xl font-extrabold text-white tracking-tight">Fuel Pricing Registry</h1>
                    <p class="text-slate-400 mt-1">Manage petroleum product rates across regional depots in Nepal.</p>
                </div>
                    ...
                    <tbody class="divide-y divide-slate-800 text-sm">
                        @foreach ($fuelPrices as $price)
                            <tr class="hover:bg-slate-800/30 transition-colors">
                                <td class="px-6 py-4 font-bold text-white">{{ $price->product }}</td>
                                <td class="px-6 py-4 text-slate-400">{{ $price->region }}</td>
                                ...
                                <td class="px-6 py-4 text-right space-x-3">
                                    <a href="{{ route('admin.fuel-prices.edit', $price) }}"
                                        class="text-indigo-400 hover:text-indigo-300 font-bold uppercase text-xs">Edit</a>
                                    <form action="{{ route('admin.fuel-prices.destroy', $price) }}" method="POST"
                                        class="inline" onsubmit="return confirm('Archive this price record?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
