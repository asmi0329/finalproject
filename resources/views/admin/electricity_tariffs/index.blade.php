    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-10">
                <div>
                    <h1 class="text-3xl font-extrabold text-white tracking-tight">Electricity Tariff Registry</h1>
                    <p class="text-slate-400 mt-1">Manage consumption tiers and slab rates for various user categories.
                    </p>
                </div>
                <a href="{{ route('admin.electricity-tariffs.create') }}"
                ...
                <table class="w-full text-left datatable">
                    <thead
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-4">Consumption Slab</th>
                            <th class="px-6 py-4">Rate (NPR)</th>
                    ...
                    <tbody class="divide-y divide-slate-800 text-sm">
                        @foreach ($electricityTariffs as $tariff)
                            <tr class="hover:bg-slate-800/30 transition-colors">
                                <td class="px-6 py-4 font-bold text-white">{{ $tariff->category }}</td>
                                <td class="px-6 py-4 text-slate-400">{{ $tariff->slab }}</td>
                                <td class="px-6 py-4 font-black text-indigo-400">Rs {{ number_format($tariff->rate, 2) }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($tariff->is_manual)
                                        <span
                                ...
                                <td class="px-6 py-4 text-right space-x-3">
                                    <a href="{{ route('admin.electricity-tariffs.edit', $tariff) }}"
                                        class="text-indigo-400 hover:text-indigo-300 font-bold uppercase text-xs">Edit</a>
                                    <form action="{{ route('admin.electricity-tariffs.destroy', $tariff) }}" method="POST"
                                        class="inline" onsubmit="return confirm('Archive this tariff?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
