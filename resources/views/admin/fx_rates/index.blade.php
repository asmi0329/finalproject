    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-10">
                <div>
                    <h1 class="text-3xl font-extrabold text-white tracking-tight">Exchange Rate Registry</h1>
                    <p class="text-slate-400 mt-1">Manage foreign currency exchange rates relative to NPR.</p>
                </div>
            ...
            <div class="admin-card overflow-hidden">
                <table class="w-full text-left datatable">
                    <thead
                        ...
                        <tr>
                            <th class="px-6 py-4">Currency Pair</th>
                            <th class="px-6 py-4">Rate (1 unit)</th>
                    ...
                    <tbody class="divide-y divide-slate-800 text-sm">
                        @foreach ($fxRates as $rate)
                            <tr class="hover:bg-slate-800/30 transition-colors">
                                <td class="px-6 py-4 font-bold text-white">{{ $rate->base_currency }} /
                                    {{ $rate->target_currency }}</td>
                                <td class="px-6 py-4 font-black text-indigo-400">{{ number_format($rate->rate, 4) }}</td>
                                <td class="px-6 py-4">
                                    @if($rate->is_manual)
                                        <span
                                ...
                                <td class="px-6 py-4 text-slate-500 text-xs">
                                    {{ $rate->fetched_at ? $rate->fetched_at->diffForHumans() : 'Never' }}
                                </td>
                                <td class="px-6 py-4 text-right space-x-3">
                                    <a href="{{ route('admin.fx-rates.edit', $rate) }}"
                                        class="text-indigo-400 hover:text-indigo-300 font-bold uppercase text-xs">Edit</a>
                                    <form action="{{ route('admin.fx-rates.destroy', $rate) }}" method="POST" class="inline"
                                        onsubmit="return confirm('Archive this rate?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
