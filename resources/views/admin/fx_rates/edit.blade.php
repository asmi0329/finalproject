    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-10 flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-extrabold text-white tracking-tight">Modify Exchange Rate</h1>
                    <p class="text-slate-400 mt-1">Adjust the currency pair rate or validation settings.</p>
                </div>
            ...
            <div class="admin-card p-8">
                <form action="{{ route('admin.fx-rates.update', $fxRate) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="base_currency"
                                value="{{ old('base_currency', $fxRate->base_currency) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="target_currency"
                                value="{{ old('target_currency', $fxRate->target_currency) }}"
                                class="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all"
                                required>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.0001" name="rate" value="{{ old('rate', $fxRate->rate) }}"
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="fetched_at"
                            value="{{ old('fetched_at', $fxRate->fetched_at->format('Y-m-d\TH:i')) }}"
                            class="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
